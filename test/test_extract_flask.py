...
def test_extract_basic(client):
    ...
    # Store 'xml'
    resp = client.post(
        "/store", data={"FileName": file_name, "XML": xml_as_string, "Password": password})
    assert resp.status_code == 200

    # Extract stored 'xml'
    # resp = client.get("/extract")
    resp = client.post(
        "/extract", data={"FileName": file_name, "Password": password})
...
def test_extract_not_there(client):
    ...
    # assert resp.status_code == 200
    resp = client.post(
        "/extract", data={"FileName": file_name + 'i', "Password": password + 'i'})
